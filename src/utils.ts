...
export function createAvatar(
  ...
  const avatarFuture = avatarFactory.createAvatarFromFile(
    "avatar.json" /*, avatarFactory?.bundledFileSystem*/
  ...
  avatarFactory
    .parseAvatarMatricesFromFile(
      "presets.json" /*, avatarFactory?.bundledFileSystem*/
    ) // uncomment to load the avatar presets from app assets
    .then(
      (presets) => {
        ...
        if (avatarPresets.length > 0) {
          presetsSwapExecutor = new PeriodicExecutor(20, () => {
            ...
            avatar
              ...
              .then(() =>
                console.log(`Updated to avatar preset ${currentPresetIndex}`)
              );
      ...
      (error) =>
        console.error(`Failed to load and parse avatar presets, ${error}`)
    );
